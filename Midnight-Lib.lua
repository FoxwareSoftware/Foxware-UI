...
local CONFIG_FOLDER = "MidnightLib"
...
local function save_config(game_name, config_name, data)
    ...
    writefile(path .. "/" .. config_name .. ".json", midnight.svc.http:JSONEncode(encoded))
end

local function load_config(game_name, config_name)
    local path = CONFIG_FOLDER .. "/" .. game_name .. "/" .. config_name .. ".json"
    if isfile(path) then
        local raw = midnight.svc.http:JSONDecode(readfile(path))
        local out = {}
...
function midnight.new(title, options)
    options = options or {}

    if getgenv then
        if getgenv().__midnight_inst then
            pcall(function() getgenv().__midnight_inst:destroy() end)
            getgenv().__midnight_inst = nil
        end
    end

    local localPlayer = midnight.svc.plr.LocalPlayer

    local existing = localPlayer.PlayerGui:FindFirstChild("Midnight")
    ...
    task.spawn(function()
        local url = "https://raw.githubusercontent.com/FoxwareSoftware/Foxware-UI/refs/heads/main/MidnightPng.png"
        local path = "midnightliblogo.png"
        ...
        midnight.svc.tw:Create(logo, TweenInfo.new(0.5), {ImageTransparency = 0}):Play()
        task.wait(3)
        midnight.svc.tw:Create(logo, TweenInfo.new(0.5), {ImageTransparency = 1}):Play()
        task.wait(0.5)
        load_gui:Destroy()
        loaded = true
    end)
    ...
    if options.KeySystem then
        ...
        local fileName = ks.FileName and (ks.FileName .. "_midnightkey.txt")
        local th = midnight.theme
        ...
        local card = make("Frame", {
            ...
            BackgroundColor3 = midnight.theme.bg,
            BackgroundTransparency = 1,
            Parent = splashGui
        })
        make("UICorner", { CornerRadius = UDim.new(0, 14), Parent = card })
        make("UIStroke", { Color = midnight.theme.border, Thickness = 1, Transparency = 1, Parent = card })

        local accent_bar = make("Frame", {
            Size = UDim2.fromOffset(3, 32),
            Position = UDim2.fromOffset(18, 22),
            BackgroundColor3 = midnight.theme.accent,
            BackgroundTransparency = 1,
            Parent = card
        })
        ...
        local splashTitle = make("TextLabel", {
            ...
            TextColor3 = midnight.theme.text,
            TextTransparency = 1,
            TextSize = 17,
            Font = Enum.Font.GothamBold,
            TextXAlignment = Enum.TextXAlignment.Left,
        ...
        local splashSub = make("TextLabel", {
            ...
            TextColor3 = midnight.theme.subtext,
            TextTransparency = 1,
            TextSize = 13,
            Font = Enum.Font.Gotham,
            TextXAlignment = Enum.TextXAlignment.Left,
        ...
        midnight.svc.tw:Create(card, fadeIn, { BackgroundTransparency = 0 }):Play()
        midnight.svc.tw:Create(card:FindFirstChildOfClass("UIStroke"), fadeIn, { Transparency = 0 }):Play()
        midnight.svc.tw:Create(accent_bar, fadeIn, { BackgroundTransparency = 0 }):Play()
        midnight.svc.tw:Create(splashTitle, fadeIn, { TextTransparency = 0 }):Play()
        midnight.svc.tw:Create(splashSub, fadeIn, { TextTransparency = 0 }):Play()
        midnight.svc.tw:Create(splashBrand, fadeIn, { TextTransparency = 0 }):Play()

        task.wait(2)

        local fadeOut = TweenInfo.new(0.4)
        midnight.svc.tw:Create(card, fadeOut, { BackgroundTransparency = 1 }):Play()
        midnight.svc.tw:Create(card:FindFirstChildOfClass("UIStroke"), fadeOut, { Transparency = 1 }):Play()
        midnight.svc.tw:Create(accent_bar, fadeOut, { BackgroundTransparency = 1 }):Play()
        midnight.svc.tw:Create(splashTitle, fadeOut, { TextTransparency = 1 }):Play()
        midnight.svc.tw:Create(splashSub, fadeOut, { TextTransparency = 1 }):Play()
        midnight.svc.tw:Create(splashBrand, fadeOut, { TextTransparency = 1 }):Play()
        task.wait(0.5)
        splashGui:Destroy()
    end
    ...
    w.is_mobile = midnight.svc.inp.TouchEnabled and not midnight.svc.inp.KeyboardEnabled

    local t = {
        bg = (options.Theme and options.Theme.bg) or midnight.theme.bg,
        container = (options.Theme and options.Theme.container) or midnight.theme.container,
        element = (options.Theme and options.Theme.element) or midnight.theme.element,
        hover = (options.Theme and options.Theme.hover) or midnight.theme.hover,
        active = (options.Theme and options.Theme.active) or midnight.theme.active,
        accent = (options.Theme and options.Theme.accent) or midnight.theme.accent,
        text = (options.Theme and options.Theme.text) or midnight.theme.text,
        subtext = (options.Theme and options.Theme.subtext) or midnight.theme.subtext,
        border = (options.Theme and options.Theme.border) or midnight.theme.border
    }
    ...
    if not w.is_mobile then
        ...
        table.insert(conns, midnight.svc.inp.InputChanged:Connect(function(inp)
        ...
        table.insert(conns, midnight.svc.inp.InputEnded:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                resizing = false
            end
        end))
        ...
        resize_handle.MouseEnter:Connect(function()
            midnight.svc.tw:Create(resize_handle, TweenInfo.new(0.2), {ImageColor3 = t.text}):Play()
        end)
        resize_handle.MouseLeave:Connect(function()
            midnight.svc.tw:Create(resize_handle, TweenInfo.new(0.2), {ImageColor3 = t.subtext}):Play()
        end)
    end
    ...
    minimize.MouseEnter:Connect(function()
        play("hover")
        midnight.svc.tw:Create(minimize, TweenInfo.new(0.2), {ImageColor3 = t.text}):Play()
    end)
    minimize.MouseLeave:Connect(function()
        midnight.svc.tw:Create(minimize, TweenInfo.new(0.2), {ImageColor3 = t.subtext}):Play()
    end)
    ...
    minimize.MouseButton1Click:Connect(function()
        ...
        if minimized then
            ...
            midnight.svc.tw:Create(main, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
        ...
        midnight.svc.tw:Create(minimize, TweenInfo.new(0.25), {
    ...
    local function update_input(input)
        ...
        midnight.svc.tw:Create(main, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Position = position
        }):Play()
    end
    ...
    table.insert(conns, midnight.svc.run.Heartbeat:Connect(function()
    ...
    if w.is_mobile then
        midnight.svc.cas:BindAction("MidnightToggle", function(name, state, input)
            if state == Enum.UserInputState.Begin then toggle_gui() end
        end, true, Enum.KeyCode.ButtonR3)
        midnight.svc.cas:SetTitle("MidnightToggle", "Midnight")
        midnight.svc.cas:SetPosition("MidnightToggle", UDim2.new(1, -70, 1, -70))
    else
        table.insert(conns, midnight.svc.inp.InputBegan:Connect(function(input, gpe)
    ...
    local function make_colorpicker_popup(current, on_change)
        ...
        local pc_conn = midnight.svc.inp.InputChanged:Connect(function(inp)
        ...
        local pe_conn = midnight.svc.inp.InputEnded:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                dragging_sv = false
                dragging_hue = false
            end
    ...
    function w:tab(name, icon)
        ...
        if has_icon then
            local icon_id = midnight.icons[icon]
            ...
            if type(icon) == "string" and icon:find("^https://") then
                task.spawn(function()
                    pcall(function()
                        ...
                        local fname = "midnightlib_icons/" .. icon:gsub("[^%w]", "_"):sub(1, 64) .. ".png"
                        if not isfolder("midnightlib_icons") then makefolder("midnightlib_icons") end
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            for _, tb in pairs(w.tabs) do
                ...
                midnight.svc.tw:Create(tb.btn, ti, {BackgroundColor3 = t.element}):Play()
                midnight.svc.tw:Create(tb.txt, ti, {TextColor3 = t.subtext}):Play()
                midnight.svc.tw:Create(tb.indicator, ti, {Size = UDim2.new(0, 0, 0, 38)}):Play()
                if tb.icon then
                    midnight.svc.tw:Create(tb.icon, ti, {ImageColor3 = t.subtext}):Play()
                end
            end

            page.Visible = true
            midnight.svc.tw:Create(btn, ti, {BackgroundColor3 = t.hover}):Play()
            midnight.svc.tw:Create(txt, ti, {TextColor3 = t.text}):Play()
            midnight.svc.tw:Create(indicator, ti, {Size = UDim2.new(0, 3, 0, 38)}):Play()
            if tab.icon then
                midnight.svc.tw:Create(tab.icon, ti, {ImageColor3 = t.text}):Play()
            end
            w.active = tab
        end)
        ...
        btn.MouseLeave:Connect(function()
            if w.active ~= tab then
                midnight.svc.tw:Create(btn, ti, {BackgroundColor3 = t.element}):Play()
            end
        end)
        ...
        function tab:button(text, callback, icon)
            ...
            click.MouseButton1Click:Connect(function()
                ...
                midnight.svc.tw:Create(ripple, TweenInfo.new(0.4), {
            ...
            click.MouseEnter:Connect(function()
                play("hover")
                midnight.svc.tw:Create(box, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
            end)
            click.MouseLeave:Connect(function()
                midnight.svc.tw:Create(box, TweenInfo.new(0.15), {BackgroundColor3 = t.element}):Play()
            end)
        ...
        function tab:toggle(text, def, callback, flag)
            ...
            local function set_state(new_state, run_callback)
                ...
                midnight.svc.tw:Create(tog_bg, ti_tog, {BackgroundColor3 = state and t.accent or t.container}):Play()
                midnight.svc.tw:Create(knob, ti_tog, {
                    Position = state and UDim2.new(1, -3, 0.5, 0) or UDim2.new(0, 3, 0.5, 0),
                    AnchorPoint = Vector2.new(state and 1 or 0, 0.5)
                }):Play()
                midnight.svc.tw:Create(lbl, ti_tog, {TextColor3 = state and t.text or t.subtext}):Play()
        ...
        function tab:slider(text, min, max, def, callback, flag, step)
            ...
            if w.is_mobile then
                ...
                minus_btn.MouseEnter:Connect(function()
                    midnight.svc.tw:Create(minus_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
                end)
                minus_btn.MouseLeave:Connect(function()
                    midnight.svc.tw:Create(minus_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.container}):Play()
                end)
                plus_btn.MouseEnter:Connect(function()
                    midnight.svc.tw:Create(plus_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
                end)
                plus_btn.MouseLeave:Connect(function()
                    midnight.svc.tw:Create(plus_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.container}):Play()
                end)
            end
        ...
        function tab:dropdown(text, options, def, callback, flag)
            ...
            local function close_dropdown()
                ...
                midnight.svc.tw:Create(container, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 42)}):Play()
                midnight.svc.tw:Create(list_frame, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 0)}):Play()
                midnight.svc.tw:Create(arrow, TweenInfo.new(0.2), {Rotation = 0}):Play()
                task.wait(0.2)
            ...
            for i, option in ipairs(options) do
                ...
                opt_btn.MouseEnter:Connect(function()
                    midnight.svc.tw:Create(opt_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
                end)
                opt_btn.MouseLeave:Connect(function()
                    midnight.svc.tw:Create(opt_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.element}):Play()
                end)
            ...
            dropdown_btn.MouseButton1Click:Connect(function()
                ...
                if open then
                    ...
                    midnight.svc.tw:Create(container, TweenInfo.new(0.2), {
                        Size = UDim2.new(1, 0, 0, 42 + 34 + max_list + 4)
                    }):Play()
                    midnight.svc.tw:Create(list_frame, TweenInfo.new(0.2), {
                        Size = UDim2.new(1, 0, 0, 34 + max_list)
                    }):Play()
                    midnight.svc.tw:Create(arrow, TweenInfo.new(0.2), {Rotation = 180}):Play()
                else
                    task.spawn(close_dropdown)
                end
            end)

            table.insert(conns, midnight.svc.inp.InputBegan:Connect(function(inp, gpe)
            ...
            dropdown_btn.MouseEnter:Connect(function()
                play("hover")
                midnight.svc.tw:Create(dropdown_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
            end)
            dropdown_btn.MouseLeave:Connect(function()
                midnight.svc.tw:Create(dropdown_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.container}):Play()
            end)
        ...
        function tab:progressbar(text, max_val)
            ...
            function api:set(val, animate)
                ...
                if animate then
                    midnight.svc.tw:Create(fill, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
        ...
        function tab:keybind(text, default, hold_to_interact, callback, flag)
            ...
            keybind_btn.MouseButton1Click:Connect(function()
                ...
                local conn
                conn = midnight.svc.inp.InputBegan:Connect(function(input, gpe)
                    if gpe then return end
            ...
            table.insert(conns, midnight.svc.inp.InputEnded:Connect(function(input, gpe)
            ...
            keybind_btn.MouseEnter:Connect(function()
                play("hover")
                midnight.svc.tw:Create(keybind_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
            end)
            keybind_btn.MouseLeave:Connect(function()
                midnight.svc.tw:Create(keybind_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.container}):Play()
            end)
    ...
    function w:create_settings()
        ...
        local function create_popup(ptitle, message, on_yes)
            local popup_gui = make("ScreenGui", {
                ...
                IgnoreGuiInset = true,
                DisplayOrder = 999999999,
                Parent = midnight.svc.plr.LocalPlayer.PlayerGui
            })
            ...
            local function create_btn(label, is_yes)
                ...
                btn.MouseEnter:Connect(function()
                    midnight.svc.tw:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
                end)
                btn.MouseLeave:Connect(function()
                    midnight.svc.tw:Create(btn, TweenInfo.new(0.15), {
                        BackgroundColor3 = is_yes and t.accent or t.element
                    }):Play()
                end)
                btn.MouseButton1Click:Connect(function()
        ...
        local function refresh_configs()
            for _, child in ipairs(settings.page:GetChildren()) do
                if child.Name == "MidnightConfigRow" then
                    child:Destroy()
                end
            end
            ...
            for _, cfg_name in ipairs(configs) do
                ...
                load_btn.MouseEnter:Connect(function()
                    midnight.svc.tw:Create(cfg_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
                end)
                load_btn.MouseLeave:Connect(function()
                    midnight.svc.tw:Create(cfg_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.element}):Play()
                end)
                delete_btn.MouseEnter:Connect(function()
                    midnight.svc.tw:Create(delete_btn, TweenInfo.new(0.15), {ImageColor3 = Color3.fromRGB(255, 100, 100)}):Play()
                end)
                delete_btn.MouseLeave:Connect(function()
                    midnight.svc.tw:Create(delete_btn, TweenInfo.new(0.15), {ImageColor3 = t.subtext}):Play()
                end)
        ...
        local preset_names = {}
        for name in pairs(midnight.presets) do table.insert(preset_names, name) end
        table.sort(preset_names)
        settings:dropdown("preset", preset_names, "midnight", function(name)
            local p = midnight.presets[name]
        ...
        for _, pair in ipairs(theme_props) do
            ...
            row.MouseEnter:Connect(function()
                midnight.svc.tw:Create(row, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
            end)
            row.MouseLeave:Connect(function()
                midnight.svc.tw:Create(row, TweenInfo.new(0.15), {BackgroundColor3 = t.element}):Play()
            end)
    ...
    function w:notify(title, text, duration)
        ...
        local notif_gui = w.localPlayer.PlayerGui:FindFirstChild("MidnightNotifications")
        if not notif_gui then
            notif_gui = make("ScreenGui", {
                Name = "MidnightNotifications",
                ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
                ResetOnSpawn = false,
                IgnoreGuiInset = true,
        ...
        local function dismiss()
            ...
            dismissed = true
            midnight.svc.tw:Create(notif, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
            midnight.svc.tw:Create(bar, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
            midnight.svc.tw:Create(title_lbl, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
            midnight.svc.tw:Create(text_lbl, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
            midnight.svc.tw:Create(prog_fill, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
            midnight.svc.tw:Create(prog_bg, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
            task.wait(0.3)
        ...
        midnight.svc.tw:Create(wrapper, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
            Position = UDim2.new(0, 0, 0, 0)
        }):Play()
        midnight.svc.tw:Create(prog_fill, TweenInfo.new(duration, Enum.EasingStyle.Linear), {
            Size = UDim2.fromScale(0, 1)
        }):Play()
    ...
    function w:destroy()
        ...
        local ng = w.localPlayer.PlayerGui:FindFirstChild("MidnightNotifications")
        if ng then ng:Destroy() end
        if w.is_mobile then
            pcall(function() midnight.svc.cas:UnbindAction("MidnightToggle") end)
        end
    end

    if getgenv then getgenv().__midnight_inst = w end
    return w
end
